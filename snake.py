...
class LarpSnake:
    def __init__(self, stdscr):
        ...
        self.base_speed = 0.095
        self.speed = 0.095
        self.sprint_mult = 1.4
    ...
    def _make_wav(self, freq: float, duration_ms: int, volume: float = 0.3) -> bytes:
        """Generate a short soft sine-wave WAV in memory."""
        sample_rate = 22050
        ...
        for i in range(n):
            ...
            if t < attack:
                env = t / attack
            elif t > duration_ms / 1000 - release:
                env = max(0.0, (duration_ms / 1000 - t) / release)
            val = env * volume * math.sin(2 * math.pi * freq * t)
    ...
    def _play_sound(self, kind: str = "normal"):
        """Soft short sounds. Works on Windows and Linux."""
        if self.muted:
            return
        try:
            ...
            # Linux / macOS — generate tiny wav and play it
            if kind == "bonus":
                tones = [(520, 45), (780, 55)]
            else:
                tones = [(460, 35)]
    ...
    def draw(self):
        ...
        if self.muted:
            tags += "  [MUTE]"
        header = f" LarpSnake  │  {level_name}  │  Score: {self.score}  │  Best: {self.high_scores.get(self.current_level, 0)}{tags} "
        self.stdscr.attron(curses.color_pair(4) | curses.A_BOLD)
        ...
        if self.paused:
            ...
            help1 = " Arrows/WASD move   C sprint   P pause   M mute   Q menu "
    ...
    def update(self):
        ...
        if ate:
            ...
            self.base_speed = max(0.042, self.base_speed - 0.003)
            if self.sprinting:
                self.speed = self.base_speed / self.sprint_mult
            else:
                self.speed = self.base_speed
